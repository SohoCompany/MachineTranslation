...
_SIMPLE_WORD_SPLIT = re.compile(b" ")
...
def simple_tokenizer(sentence):
	words = []
	for space_separated_fragment in sentence.strip().split():
		words.extend(_SIMPLE_WORD_SPLIT.split(space_separated_fragment))
	return [w for w in words if w]
...
def prepare_data_test(data_dir, from_train_path, to_train_path, from_dev_path, to_dev_path, from_vocabulary_size, to_vocabulary_size, tokenizer=simple_tokenizer):
	to_vocab_path = os.path.join(data_dir, "vocab%d.to" % to_vocabulary_size)
	from_vocab_path = os.path.join(data_dir, "vocab%d.from" % from_vocabulary_size)
	#create_vocabulary(to_vocab_path, to_train_path, to_vocabulary_size, tokenizer, False)
	#create_vocabulary(from_vocab_path, from_train_path, from_vocabulary_size, tokenizer, False)

	to_train_ids_path = to_train_path + (".ids%d" % to_vocabulary_size)
	from_train_ids_path = from_train_path + (".ids%d" % from_vocabulary_size)
	#data_to_token_ids(to_train_path, to_train_ids_path, to_vocab_path, tokenizer, False)
	#data_to_token_ids(from_train_path, from_train_ids_path, from_vocab_path, tokenizer, False)

	to_dev_ids_path = to_dev_path + (".ids%d" % to_vocabulary_size)
	from_dev_ids_path = from_dev_path + (".ids%d" % from_vocabulary_size)
	#data_to_token_ids(to_dev_path, to_dev_ids_path, to_vocab_path, tokenizer, False)
	#data_to_token_ids(from_dev_path, from_dev_ids_path, from_vocab_path, tokenizer, False)
